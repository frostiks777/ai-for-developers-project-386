...
- `/dashboard` — организатор: список броней с отменой и настройки доступности (вход по паролю, см. [«Доступ организатора»](#доступ-организатора-пароль)).
...
Панель организатора (`/dashboard` и `/admin/*`) закрыта HTTP Basic Auth. Пароль задаётся переменной `ADMIN_PASSWORD`; имя пользователя — любое. Тот же пароль требуется для изменяющих админ-запросов (настройки доступности, типы встреч, блокировки); публичные чтения для гостей (слоты, типы, доступность, брони для `/events`) открыты.
