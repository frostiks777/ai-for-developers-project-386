...
Пример — [`.env.example`](.env.example). При первом старте создаются таблицы и 8 слотов на ближайшие 4 дня (10:00 и 15:00, по 30 минут).
...
| `POST` | `/api/bookings` | создать бронь: `slotId`, `name`, `phone`, `email` |
...
Ошибки: `400` — невалидное тело или прошедший слот, `404` — слот не найден, `409` — слот уже занят.
