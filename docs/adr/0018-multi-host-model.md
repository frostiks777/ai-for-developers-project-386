...
**−** Скаляры расписания (`slotDurationMin`, буферы, `minNoticeMin`, `horizonDays`) пока общие — хранятся в одной строке `availability_rules`; per-host — следующий шаг. Слоты при этом уже per-host (различаются диапазоны `availability_ranges`).
