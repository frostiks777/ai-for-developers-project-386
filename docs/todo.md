...
- [ ] **Перевести чекбоксы в Switch/тоггл** (§3, §7): сейчас нативный `input[type=checkbox]`, спека просит аккуратный тоггл.
- [ ] **Иконка `+` вместо текста «Добавить интервал»** (§3, §7): кнопка сейчас текстовая (`Button size="sm"`).
- [ ] **Кнопка «Копировать» интервал дня** (§3, §7): иконка `Copy` + поповер выбора целевых дней (Bulk copy). Сейчас нет.
- [ ] **Быстрые пресеты (Preset Chips)** (§4, §7): `Пн–Пт 10:00–18:00`, `Пн–Пт 09:00–18:00`, `Каждый день 10:00–20:00`, `Очистить всё`. Сейчас нет.
- [ ] **Индикатор таймзоны в шапке карточки** (§2, §7): строка `Часовой пояс: <IANA>`. Сейчас `timeZone` в настройках есть, но в UI карточки не показан.
- [ ] **Real-time валидация** (§5): хронология (`start < end` → красный бордер + подсказка), пересечение интервалов одного дня, мин. длительность по активным типам встреч. Сейчас только блокировка кнопки/серверная zod-проверка.
- [ ] **Информационная подсказка о пересечении слотов** в подвале (§2, §4).
- [ ] **Undo для пресетов** (§4): применение пресета с возможностью отмены через toast (или подтверждение при кастомных правках).
- [ ] **Учесть маршрут `/admin/availability`** (из P1-дашборда): сейчас блок живёт на `/dashboard` секцией `#availability`.
...
- [ ] **Цель: список слотов помещается в колонку целиком** — сетка слотов должна ужиматься по доступной высоте карточки (без обрезки снизу и без уезжающего верха), заголовок дня остаётся видимым.
- [ ] **Фолбэк, если целиком не влезает:** внутренний скролл **только внутри списка слотов**, в стиле страницы (тонкий/скрытый нативный скроллбар, как в остальном UI) — контейнер списка `flex-1 min-h-0 overflow-y-auto`; скроллится список, а не вся страница/карточка. Последний слот доступен, заголовок не уезжает.
- [ ] Компонент — `src/components/slot-grid.tsx` (или колонка в `home-page.tsx`). Тест: RTL-смоук на `overflow-y-auto` у контейнера списка; визуальная проверка на 1280×820 и 390×844, обе темы.
